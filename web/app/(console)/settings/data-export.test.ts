...
const source =
  ...
  readFileSync(join(process.cwd(), "app/(console)/settings/page.tsx"), "utf8") +
  readFileSync(join(process.cwd(), "lib/job-events.ts"), "utf8");
...
assert.doesNotMatch(readFileSync(join(process.cwd(), "components/settings/data-export-card.tsx"), "utf8"), /hasPendingDumps|refetchInterval/);
assert.match(source, /useJobEvents/);
assert.match(source, /new EventSource\(currentUserEventsURL\(\), \{ withCredentials: true \}\)/);
