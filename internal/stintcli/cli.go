...
func skipLeadingPositionals(args []string) []string {
...
func importCommonArgs(args []string) []string {
	out := make([]string, 0, len(args))
	for i, arg := range args {
		...
		switch arg {
		case "--client-id", "--client-secret", "--code", "--redirect-uri", "--refresh-token":
			if i+1 < len(args) {
				continue
			}
		}
		...
		if strings.HasPrefix(arg, "--ids=") {
			continue
		}
		if strings.HasPrefix(arg, "--client-id=") ||
			strings.HasPrefix(arg, "--client-secret=") ||
			strings.HasPrefix(arg, "--code=") ||
			strings.HasPrefix(arg, "--redirect-uri=") ||
			strings.HasPrefix(arg, "--refresh-token=") {
			continue
		}
		if !strings.HasPrefix(arg, "-") && len(out) == 0 {
			continue
		}
		out = append(out, arg)
	}
...
func devCommonArgs(args []string) []string {
	out := make([]string, 0, len(args))
	for i, arg := range args {
		if i == 0 && !strings.HasPrefix(arg, "-") {
			continue
		...
		switch arg {
		case "--github-id", "--username", "--retention-days", "--range", "--now-unix":
			if i+1 < len(args) {
				continue
			}
		}
		...
		if strings.HasPrefix(arg, "--github-id=") ||
			strings.HasPrefix(arg, "--username=") ||
			strings.HasPrefix(arg, "--retention-days=") ||
			strings.HasPrefix(arg, "--range=") ||
			strings.HasPrefix(arg, "--now-unix=") {
			continue
		}
		out = append(out, arg)
	}
...
func publicReadCommonArgs(args []string) []string {
	filtered := make([]string, 0, len(args))
	endpointFlags := map[string]bool{
		"--country":  true,
		"--end":      true,
		"--language": true,
		"--range":    true,
	...
	for i := 0; i < len(args); i++ {
		...
		if strings.HasPrefix(arg, "--country=") ||
			strings.HasPrefix(arg, "--end=") ||
			strings.HasPrefix(arg, "--language=") ||
			strings.HasPrefix(arg, "--range=") ||
			strings.HasPrefix(arg, "--start=") {
			continue
		}
		if endpointFlags[arg] {
			i++
			continue
		}
		filtered = append(filtered, arg)
	}
	return skipLeadingPositionals(filtered)
}
